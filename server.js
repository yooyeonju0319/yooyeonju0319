...
app.post("/api/signup", async (req, res) => {
  try {
    ...
    await db.query(
      "INSERT INTO users (username, password, question, answer, profilePic) VALUES ($1, $2, $3, $4, $5)",
...
app.post("/api/login/recover", async (req, res) => {
  try {
    ...
    const result = await db.query("SELECT * FROM users WHERE username = $1 AND answer = $2", [username, answer]);
    const user = result.rows[0];
...
app.post("/api/photos/upload", upload.single("photo"), async (req, res) => {
  try {
    ...
    await db.query(
      "INSERT INTO photos (uploader, url, title, tags, description, likes) VALUES ($1, $2, $3, $4, $5, $6)",
      [uploader, url, title, tags, description, JSON.stringify([])]
    );

    const result = await db.query("SELECT * FROM photos WHERE url = $1", [url]);
    console.log("✅ DB 저장 완료!");

    res.status(201).json({ message: "사진이 업로드되었습니다.", photo: result.rows[0] });
  } catch (error) {
